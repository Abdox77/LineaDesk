...
import type { ProjectResponseDto, ProjectState } from '../api/types';

interface EditProjectModalProps {
    project: ProjectResponseDto;
    onSave: (data: { projectName: string; description: string; githubLink: string; state?: ProjectState }) => Promise<void>;
    onClose: () => void;
}
...
export function EditProjectModal({ project, onSave, onClose }: EditProjectModalProps) {
    ...
    const [state, setState] = useState<ProjectState>(project.state ?? 'PENDING');
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm" onClick={onClose}>
            ...
            >
                ...
                <form onSubmit={handleSubmit} className="p-6 flex flex-col gap-5">
                    ...
                    <div className="flex flex-col gap-1.5">
                        <label className="text-sm font-semibold text-slate-700 dark:text-slate-300">GitHub Link</label>
                        <input
                            className="form-input w-full rounded-lg border border-slate-300 dark:border-slate-700 bg-slate-50 dark:bg-[#0d1117] h-10 px-3 text-sm text-slate-900 dark:text-white placeholder:text-slate-400 focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary"
                            value={githubLink}
